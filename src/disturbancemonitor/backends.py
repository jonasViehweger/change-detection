...
class FreeCDSEProcessAPI(Backend):
    def __init__(
        ...
        s3_profile: str | None = None,
        sh_profile: str = "default-profile",
        byoc_sh_profile: str = "default-profile",
        monitor_id: str | None = None,
        rollback: bool = True,
        config: GeoConfigHandler | None = None,
    ) -> None:
        """
        Special class of ProcessAPI

        Free Users on CDSE do not have access to BYOC collections.
        This class exists to enable free Users to use their own quota on processing, but a different "host" account
        to handle the upload to BYOC.
        """
        super().__init__(monitor_params, config)
        self.urls = Endpoints[monitor_params.endpoint].value
        self.url = self.urls.base_url + "/api/v1/process"
    ...
    def init_model(self) -> None:
        ...
        with ResourceManager(rollback=self.rollback, on_failure_callback=set_failed_status) as manager:
            ...
            print("2/6 BYOC")
            self.byoc_id = self.byoc.create_byoc()
            self.byoc.share_byoc(self.account_id)
            manager.add_resource(self.byoc)
            for feature in self.geometries.iterfeatures():
                feature_id = feature["properties"][FEATURE_ID_COLUMN]
                geometry = feature["geometry"]
