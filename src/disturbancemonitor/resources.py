...
class SHConfiguration(Resource):
    ...
    def _post_layer(self, layer: dict) -> None:
        response = self.client.post(f"{self.instances_url}/{self.instance_id}/layers", json=layer)
        try:
            response.raise_for_status()
        except HTTPError as e:
            print(f"Request failed: {e.response.status_code} - {e.response.text}")
    ...
    def create_instance(self) -> str:
        ...
        with DATA_PATH.joinpath("true_color.cjs").open("r") as f:
            true_color_evalscript = f.read()
        self._post_layer(
            {
                "title": "TRUE-COLOR",
                "id": "TRUE-COLOR",
                ...
                "styles": [
                    {"name": "default", "description": "Default layer style", "evalScript": true_color_evalscript}
                ],
